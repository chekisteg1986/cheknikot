...
class PathMapBase 
{
	...
	*/
	...
	public function fill_all_passages():Void
	{
		var _walls:FlxTilemap = LocalGame.state.walls;
		var _y:Int = _walls.heightInTiles;
		
		while (--_y >= 0)
		{
			var _x:Int = _walls.widthInTiles;
			while (--_x >= 0)
			{
				if (chkd[_y * _walls.widthInTiles + _x] == false)
				if ( can_walk_here(_x, _y)) fill_passages(_x, _y);
			}
		}
	}
			...
			while (++x <= max_x)
			{
				...
				if (passages[index])
				{
					...
					if (square) 
					{
						...
						xx = x + _size;
						if (xx > max_x) continue;
						
						yy = y + _size;
						if (yy > max_y) continue;
						...
						while (--xx >= x)
						{
							...
							{
								index2 = xx + yy * width;
								if (way_zones[index2] != null) placed  = false;
								if (!passages[index2]) placed  = false;
							}
	...
	public  function get_way_zone(p:FlxPoint):WaySection
	{
		...
		if (x <= 0) return null;
		if (y <= 0) return null;
		if (x >= pixel_width ) return null;
		if (y >= pixel_height) return null;
	...
	public function close_sections(p1:FlxPoint, p2:FlxPoint):Bool
	{
		...
		if (ws1.connected_with.indexOf(ws2) != -1) return true;
	...
	public function get_trail_to(p1:FlxPoint, p2:FlxPoint,path_memory:PathfindMemory, force:Bool = false):PathfindMemory
	{
		...
		if (ws1 == null || ws2 == null) return null;
