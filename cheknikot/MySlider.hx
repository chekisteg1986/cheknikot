 ...
 * @author Gama11
 */
class MySlider extends FlxSpriteGroup
{
	/**
	...
	private var coordinates:FlxPoint;
	...
	{
		#if !FLX_NO_MOUSE
		coordinates = FlxG.mouse.getScreenPosition();
		mousepressed = FlxG.mouse.pressed;
		#else
		touch = FlxG.touches.getFirst();
		if (touch != null)
		{
			mousepressed = true;
			coordinates = touch.getScreenPosition();
		}
		#end

		// Clicking and sound logic

		if (coordinates.inRect(_bounds))
		...
		if (mousepressed && coordinates.inRect(_bounds))
		{
			updateValue();
		}
