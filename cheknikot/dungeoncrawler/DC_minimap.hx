...
class DC_minimap 
{
	...
	public function create_tilemap(_x:Float,_y:Float):Void
	{
		//if (_as == null) return;
		
		if (mm_tilemap != null) return;//remove(mm_tilemap, true);
		...
		while (--_len >= 0) _arr.push(0);
		
		
		mm_tilemap.loadMapFromArray(_arr, _size, _size, AssetPaths.minimap_tileset__png, GameParams.TILE_SIZE, GameParams.TILE_SIZE);
	...
	public function update():Void 
	{
		...
		arrow.angle = 90 * LocalGame.state.screen_3d.camera_face;
		var _x:Int = range+1;
		var _tile_x:Int;
		var _tile_y:Int;
		
		if (connected_to == null) return;
