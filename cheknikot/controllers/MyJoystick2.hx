...
class MyJoystick2 extends FlxSpriteGroup
{
	...
	public function new(MaxSize:Int=0) 
	{
		...
		//joystick.makeGraphic(6, 6,FlxColor.BLACK);
	...
	override public function update(elapsed:Float):Void 
	{
		...
		var mouse_p:FlxPoint = null;
			...
			pressed = (touch != null);
			if (touch != null) mouse_p = touch.getScreenPosition();
		#end
		...
		if (pushed)
		{
			...
			if (mouse_p.x < 0) mouse_p.x = 0;
			if (mouse_p.x > 50) mouse_p.x = 50;
			if (mouse_p.y < 0) mouse_p.y = 0;
			if (mouse_p.y > 50) mouse_p.y = 50;
			
			
			joystick.x = x + mouse_p.x;
			joystick.y = y + mouse_p.y;
			
			if (mouse_p.x < 23) left = true;
			if (mouse_p.x > 27) right = true;
			if (mouse_p.y < 23) down = true;
			if (mouse_p.y > 27) up = true;
