...
class QR_Message extends QuestResult 
{
	...
	{
		return super.get_save_array();
	}
	override function get_save_vars():Array<String>
	{
		return save_vars;
	}*/
	public function add_message(text:Array<String>):Void
	{
		if (message == null) message = new Array();
		//message.push(face);
		message.push(text);
	}
	...
	public function next_message():Void
	{
		...
		if (message_i < message.length)
		{
			...
			//GameMessage.messanger.set_text(_text, [['ok']],[next_message], _face);
			message_i++;
			//trace('result message end');
		}
		else
		{
			//GameMessage.last_message = true;
			//GameMessage.ok_answer();
			//GameObjectLayer.game_goes_true();
		}
	}
